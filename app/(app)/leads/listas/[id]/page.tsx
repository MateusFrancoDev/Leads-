...
export async function generateMetadata({ params }: PageProps<"/leads/listas/[id]">): Promise<Metadata> {
  const { id } = await params;
  const list = await findProspectingList(id);
  return { title: list?.name ?? "Lista" };
}

export default async function ListDetailPage({ params, searchParams }: PageProps<"/leads/listas/[id]">) {
  const { id } = await params;
  const list = await findProspectingList(id);
  if (!list) notFound();
